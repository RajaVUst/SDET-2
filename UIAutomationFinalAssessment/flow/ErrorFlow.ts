...
export class ErrorFlow {
  ...
  readonly check : CheckoutPage;
  readonly pay : PaymentPage;
  ...
  constructor(
    ...
    this.check = new CheckoutPage(page,log,testInfo,evidence);
    this.pay =  new PaymentPage(page,log,testInfo,evidence);
  }
    ...
    {
        await this.cart.checkoutButtonClick();
    }

    async visibleCheckoutHeading()
    {
        await this.check.visibleCheckoutHeading();
    }
    async fillContactAndContinue()
    {
        await this.check.fillContactAndContinue();
    }
  ...
  async runErrorValidatioFlow() {
   ...
   await this.checkoutButtonClick();
